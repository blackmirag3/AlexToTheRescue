...
void sendStatus()
{
  ...
  statusPacket.params[13] = (uint32_t)(envred);
  statusPacket.params[14] = (uint32_t)(envgreen);
  statusPacket.params[15] = (uint32_t)(envblue);
...
void startMotors()
{
  TCCR0A |= (1 << 7) | (1 << 5); // enable (1 << 7) OC0A LF, (1 << 5) OC0B LR
  TCCR1A |= (1 << 5); // enable (1 << 5) OC1B RR
  TCCR2A |= (1 << 7); // enable (1 << 7) OC2A RF
}
